...
export function VersionMatrix() {
  ...
  const dynamicStats = [
    {
      label: "CPU Usage",
      value: numActive > 0 ? totalCpuStr : "—",
      sub: numActive > 0 ? "Process load" : "Server off",
      icon: Activity,
      details: cpuDetails
    },
    {
      label: "Total RAM Used",
      value: numActive > 0 ? totalRamStr : "—",
      sub: numActive > 0 ? "Allocated to JVM" : "Not allocated",
      icon: HardDrive,
      details: ramDetails
    },
    {
      label: "Avg. Tick Rate",
      value: numActive > 0 ? stats.tps : "—",
      sub: numActive > 0 ? "TPS (20.0 max)" : "No servers",
      icon: Server
    },
    {
      label: "Java Runtime",
      value: javaVersion ?? "—",
      sub: javaVersion ? "Detected" : "Checking...",
      icon: Cpu
    },
  ]
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <Icon className={cn(
                  "size-4",
                  s.value !== "—" ? "text-primary" : "text-muted-foreground"
                )} />
              </div>

              <p className={cn(
                "mt-2 font-heading text-2xl font-semibold tracking-tight",
                s.value !== "—" ? "text-primary" : "text-card-foreground"
              )}>
                {s.value}
              </p>
      ...
      <div id="quick-create">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          {CORES.map((core, i) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-3 font-heading text-sm font-semibold text-card-foreground">
                  {core.name}
                </p>
          ...
          >
            ...
            <p className="mt-3 font-heading text-sm font-semibold text-card-foreground group-hover:text-primary transition-colors">
              {t('matrix.custom_core')}
            </p>
        ...
        <div className="divide-y divide-border">
          {rows.map((row) => (
            <div
              ...
              key={row.version}
              className="grid grid-cols-[1.2fr_repeat(4,1fr)] items-center gap-2 px-4 py-2.5"
            >
